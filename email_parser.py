...
def extract_articles(email):
    ...
    7. If the article does not meet any of the criteria or lacks a description, return it with is_other set to true
    Email content:
    {email.text or email.html}

    Format the output as a JSON array of objects, each with 'title', 'description', 'url', 'is_other', and 'criteria' keys.
    ...
    try:
        ...
        other_articles = [article for article in articles if article.get('is_other', True)]
        if len(other_articles) > 0:
            logger.info(f"Starting to crawl {len(other_articles)} other articles")
            for article in other_articles:
                summary = crawl_and_summarize(article.get('url'))
                article['summary'] = summary

        other_article_prompt_content = "__".join([f"{article.get('title')},{article.get('url')},{article.get('summary')}." for article in other_articles])
        second_prompt = f"""
                Analyze the following content and extract information about articles mentioned.
                For each article:
                1. Extract the original title, description (if available), and URL (look for [LINK: url] in the text)
                2. Rewrite the title and description in a friendlier, lighter tone with a touch of personal feel
                3. Keep the rewritten content concise and engaging
                ...
                List of articles with title, link, and summary:
                {other_article_prompt_content}
        ...
        # for article in other_articles:
        #     with open(f"articles/{article.get('title')}_original.json", "a") as file:
        #         json.dump(article, file)
        #         file.write("\n")
        #         if not all(key in article for key in ('title', 'description', 'url')):
        #             raise ValueError(f"Invalid article structure: {article}")
        try:
            second_json_match = re.search(r'\[.*\]', second_response.text, re.DOTALL)
            if second_json_match:
                second_json_str = second_json_match.group(0)

                second_articles = json.loads(second_json_str)
                articles.extend(second_articles)
                # for article in second_articles:
                #     with open(f"articles/{article.get('title')}.json", "a") as file:
                #         json.dump(article, file)
                #         file.write("\n")
                #         if not all(key in article for key in ('title', 'description', 'url')):
                #             raise ValueError(f"Invalid article structure: {article}")
        ...
        articles = [article for article in articles if not article.get('is_other', True)]
